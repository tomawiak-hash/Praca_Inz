...
from logic_ai import koryguj_tresc_szkolenia
    ...
    if st.button("🚀 Generuj kompletne szkolenie"):
        if not wybrany_zawod_nazwa:
            ...
            with st.spinner(f"Tworzenie materiałów dla: {wybrany_zawod_nazwa}..."):
                ...
                # --- TUTAJ JEST ZMIANA (Łączenie środowisk) ---
                # Tworzymy jedną zmienną tekstową z obu pól
                srodowisko_full = srodowisko_glowne
                if srodowiska_dodatkowe:
                    # Dodajemy po przecinku wybrane dodatkowe miejsca
                    lista_dodatkowych = ", ".join(srodowiska_dodatkowe)
                    srodowisko_full += f" oraz okresowo: {lista_dodatkowych}"
                ...
                if "Błąd:" in opis_zawodu:
                    ...
                    # --- TUTAJ PRZEKAZUJESZ srodowisko_full ---
                    finalna_tresc = generuj_kompletne_szkolenie(
                        nazwa_firmy, 
                        wybrany_zawod_nazwa, 
                        opis_zawodu, 
                        dodatkowe_zagrozenia,
                        obowiazki,        # Pamiętaj o tym argumencie
                        srodowisko_full   # <--- TU WSTAWIASZ NOWĄ ZMIENNĄ
                    )
...
# --- Etap 2: Weryfikacja i Edycja Programu ---
    ...
    # 1. EDYTOR HARMONOGRAMU (Restrykcyjny)
    ...
    if st.session_state.tematyka_z_godzinami:
        ...
        # num_rows="fixed" <--- BLOKADA DODAWANIA/USUWANIA WIERSZY
        ...
        # Walidacja sumy godzin
        col_sum, col_warn = st.columns([1, 3])
        col_sum.caption(f"📊 Razem: **{total_h:.1f} h**")
        
        if total_h < 5.0: # Przyjmijmy min. 5h dla całości (3 ogólny + 2 stanowiskowy)
    ...
    st.subheader("🤖 Asystent Korekty")
    with st.container(border=True):
        st.write("Jeśli jakaś część szkolenia Ci nie odpowiada (np. scalenie punktów, styl), opisz to poniżej, a AI poprawi tekst.")
        uwagi_do_korekty = st.text_area("Co poprawić w treści?", placeholder="Np. Połącz punkty o ergonomii w jeden akapit. Dodaj więcej o pracy na drabinie.")
        
        if st.button("✨ Preredaguj treść z uwzględnieniem uwag"):
            if uwagi_do_korekty:
                with st.spinner("AI nanosi poprawki..."):
                    nowa_tresc = koryguj_tresc_szkolenia(st.session_state.finalna_tresc, uwagi_do_korekty)
                    if "Błąd" not in nowa_tresc:
                        st.session_state.finalna_tresc = nowa_tresc
                        st.rerun()
                    else:
                        st.error(nowa_tresc)
            else:
                st.warning("Wpisz uwagi przed kliknięciem.")
...
# --- SEKCJA DANYCH WSPÓLNYCH ---
    with st.container(border=True):
        ...
        # 1. UCZESTNICY
        st.markdown("**Lista uczestników** \n*Wpisz tylko: Imię Nazwisko, Data Urodzenia (Resztę danych system uzupełni automatycznie)*", unsafe_allow_html=True)
        ...
        # Parsowanie i Walidacja Uczestników (Z DETALAMI BŁĘDÓW)
        uczestnicy_dane_lista = []
        bledne_linie_detale = [] # Lista na konkretne komunikaty błędów
        ...
        if uczestnicy_input:
            ...
            for i, linia in enumerate(lines):
                ...
                # Walidacja liczby elementów (Teraz tylko 2: Imię, Data)
                if len(czesci) != 2:
                    bledne_linie_detale.append(f"❌ Linia {i+1}: Nieprawidłowy format ('{linia_clean}'). Wymagane 2 elementy oddzielone przecinkiem: 'Imię Nazwisko, Data'.")
                    continue

                # Walidacja formatu daty (DD.MM.RRRR)
                data_raw = czesci[1]
                if not re.match(r"^\d{2}\.\d{2}\.\d{4}$", data_raw):
                    bledne_linie_detale.append(f"❌ Linia {i+1}: Zły format daty '{data_raw}'. Wymagane DD.MM.RRRR (np. 01.03.1985).")
                    continue
                
                # Jeśli wszystko OK -> Dodajemy do listy
                uczestnicy_dane_lista.append({
                    'index': len(uczestnicy_dane_lista) + 1, 
                    'imie_nazwisko': czesci[0], 
                    'miejsce_pracy': st.session_state.zapisana_firma, # Pobieramy z sesji (Krok 1)
                    'funkcja': st.session_state.wybrany_zawod,        # Pobieramy z sesji (Krok 1)
                    'data_urodzenia': czesci[1], 
                    'ocena': '', 
                    'uwagi': ''
                })

        # Wyświetlanie szczegółowych błędów
        if bledne_linie_detale:
            st.error(f"Znaleziono błędy w {len(bledne_linie_detale)} wierszach:")
            for blad in bledne_linie_detale:
                st.text(blad) # Wyświetla konkretny komunikat błędu
        
        # Wyświetlanie poprawnej tabeli
